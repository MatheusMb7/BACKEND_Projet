...
router.post('/departamentos', validarDepartamento, validarID, async (req, res, next) => {
  const departamentoCriado = await DepartamentoModel.create(req.body)
  res.status(201).json(departamentoCriado)
})

router.put('/departamentos/:id', validarDepartamento, validarID, async (req, res, next) => {
